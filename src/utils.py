...
def get_stage_info(stage):
    ...
    stage_info = {
        1: {
            ...
            "color": "#28a745",
            "description": "Kidney damage with normal function",
        },
        2: {
            "name": "Stage 2",
            "severity": "Mild",
...
def create_stage_probability_plot(probabilities):
    ...
    bars = ax.bar(
        stages, probs, color=colors, alpha=0.8, edgecolor="black", linewidth=1.5
    )
    ...
    ax.set_title(
        "Predicted Stage Probability Distribution",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def create_egfr_gauge(egfr, predicted_stage):
    ...
    ax.set_title(
        "Kidney Function Assessment - eGFR Gauge",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def create_info_plot():
    ...
    # Title
    ax.set_title(
        "Chronic Kidney Disease (CKD) Stages Reference",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
    ...
    for i, stage_data in enumerate(stages):
        ...
        ax.text(
            0.08, y_pos + 0.03, stage_text, fontsize=14, fontweight="bold", va="center"
        )
