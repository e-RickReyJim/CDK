...
class TestPredictCKD:
    ...
    def test_predict_ckd_with_valid_inputs(
        self, mock_feature_info, mock_all_models, mock_best_model, mock_pca_pipeline,
        ...
        mock_best_model.predict_proba = Mock(return_value=np.array([[0.05, 0.10, 0.60, 0.20, 0.05]]))
        
        mock_all_models.items = Mock(return_value=[
    ...
    def test_predict_ckd_output_types(self, sample_patient_normal):
        ...
        if not os.path.exists(os.path.join(os.path.dirname(__file__), '..', 'models', 'best_model.pkl')):
...
class TestInputValidation:
    ...
    def test_numeric_features_accept_floats(self):
        ...
        numeric_values = [
            48.0, 80.0, 1.020, 0.0, 0.0,
            121.0, 36.0, 1.2, 138.0, 4.6,
            15.4, 44.0, 7800.0, 5.2
        ]
...
class TestOutputFormat:
    ...
    def test_output_tuple_structure(
        self, mock_info_plot, mock_agreement_plot, mock_gauge,
        mock_prob_plot, mock_feature_info, mock_all_models,
        mock_best_model, mock_pca_pipeline, sample_patient_normal
    ):
