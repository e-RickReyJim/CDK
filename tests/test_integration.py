...
class TestFullPredictionPipeline:
    ...
    def test_complete_pipeline_stage1(self, sample_patient_normal):
        ...
        egfr = calculate_egfr(
            sample_patient_normal["serum_creatinine"], sample_patient_normal["age"]
        )
    ...
    def test_complete_pipeline_stage5(self, sample_patient_stage5):
        ...
        egfr = calculate_egfr(
            sample_patient_stage5["serum_creatinine"], sample_patient_stage5["age"]
        )
        assert egfr is not None
    ...
    def test_batch_prediction_consistency(self, sample_patients_batch):
        ...
        for patient in sample_patients_batch:
            egfr = calculate_egfr(patient["serum_creatinine"], patient["age"])
        ...
        X_extreme = np.array(
            [
                [
                    ...
                    0,
                    0,
                    0,  # Age, BP, SG, albumin, sugar, categoricals
                    157,
                    135,
                    7.8,
                    130,
                    6.2,  # BGlucose, Urea, Creatinine, Na, K
                    8.9,
                    28,
                    12000,
                    3.2,  # Hb, PCV, WBC, RBC
...
class TestDataFlow:
    ...
    def test_feature_mapping_consistency(self, sample_patient_normal):
        ...
        mapped_features = {
            "red_blood_cells": categorical_mapping[
                sample_patient_normal["red_blood_cells"]
            ],
            "pus_cell": categorical_mapping[sample_patient_normal["pus_cell"]],
            "pus_cell_clumps": categorical_mapping[
                sample_patient_normal["pus_cell_clumps"]
            ],
            ...
            "diabetes_mellitus": categorical_mapping[
                sample_patient_normal["diabetes_mellitus"]
            ],
            "coronary_artery_disease": categorical_mapping[
                sample_patient_normal["coronary_artery_disease"]
            ],
...
class TestErrorRecovery:
    ...
    def test_graceful_handling_of_edge_cases(self, edge_case_inputs):
        ...
        for case_name, inputs in edge_case_inputs.items():
            egfr = calculate_egfr(inputs["serum_creatinine"], inputs["age"])

            # Should return None for invalid inputs
            if (
                inputs["age"] <= 0
                or inputs["serum_creatinine"] <= 0
                or inputs["age"] is None
                or inputs["serum_creatinine"] is None
            ):
                assert egfr is None
